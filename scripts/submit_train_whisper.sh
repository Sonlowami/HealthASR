...
# Job TMPDIR is set inside the job to a SHORT /tmp path (AF_UNIX socket limit ~108 chars).
# Do not use long /project/.../tmp here — DataLoader workers will crash.

mkdir -p "$RUN_ROOT" "$HF_HOME/hub" "$WANDB_DIR" "$TORCH_HOME" "$XDG_CACHE_HOME"
...
# Short node-local temp (not \$HOME, not long /project path)
